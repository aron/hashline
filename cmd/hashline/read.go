...
ARGUMENTS
  <path>       Path to the file to read. Use /workspace/... paths inside the
               container. The file must be readable by the current user.
...
func cmdRead(args []string) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			fatalf("file not found: %s\nCheck the path is correct and uses /workspace/... inside the container.", path)
		}
